...
contract DogechainTemplateToken is ERC20, Ownable, TimeLock {
    ...
    uint256 public numTokensSellToAddToDOGE = 5000 * 10**_decimals;
    uint256 public _marketingReserves = 0;

    address public marketingWallet;
    address public DEAD = 0x000000000000000000000000000000000000dEaD;
    
    mapping (address => bool) public _exemptList; //Always exempt from tax (overrides _isTaxed), for owner adding initial LP etc
    mapping (address => bool) public _isTaxed; //Any LPs intended to be taxed
    ...
    event SwapThresholdsChanged(
        ...
        uint256 previousNumTokensSellToAddToDOGE, 
        uint256 newNumTokensSellToAddToLiquidity, 
        uint256 newNumTokensSellToAddToDOGE
    );
    ...
    constructor(address _primaryRouter, address _marketingWallet) ERC20(_name, _symbol) {
        _mint(msg.sender, (_supply * 10**_decimals));
        changeMarketingWallet(_marketingWallet);

        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(_primaryRouter);

        uniswapV2Router = _uniswapV2Router;

        _exemptList[address(uniswapV2Router)] = true;
        _exemptList[msg.sender] = true;
        _exemptList[marketingWallet] = true;
    }
    ...
    function _transfer(address from, address to, uint256 amount) internal override {
        ...
        if (!_exemptList[from] || !_exemptList[to]) {
            ...
            if (to == uniswapV2Pair && !inSwapAndLiquify) {
                ...
                if (_marketingReserves >= numTokensSellToAddToDOGE) {
                    _swapTokensForDOGE(numTokensSellToAddToDOGE);
                    _marketingReserves -= numTokensSellToAddToDOGE;
                    bool sent = payable(marketingWallet).send(address(this).balance);
                    require(sent, "Failed to send wDOGE");
                }
            }
        }
    ...
    function _swapTokensForDOGE(uint256 tokenAmount) private lockTheSwap {
    ...
    function _addLiquidity(uint256 tokenAmount, uint256 dogeAmount) private lockTheSwap
    {
        _approve(address(this), address(uniswapV2Router), tokenAmount);

        uniswapV2Router.addLiquidityETH{value: dogeAmount}(
            address(this),
            tokenAmount,
            0,
            0,
    ...
    function changeMarketingWallet(address newWallet) public onlyOwner returns(bool)
    {
        ...
        require(newWallet != 0x000000000000000000000000000000000000dEaD, "New marketing wallet cannot be the dead address");
        marketingWallet = newWallet;
        emit MarketingWalletChanged(marketingWallet, newWallet);
        return true;
    }

    function changeSwapThresholds(uint256 _numTokensSellToAddToLiquidity, uint256 _numTokensSellToAddToDOGE) external onlyOwner returns (bool)
    {
        require(_numTokensSellToAddToLiquidity <= _supply / 100, "Cannot liquidate more than 1% of the supply at once!");
        require(_numTokensSellToAddToDOGE <= _supply / 100, "Cannot liquidate more than 1% of the supply at once!");
        numTokensSellToAddToLiquidity = _numTokensSellToAddToLiquidity * 10**_decimals;
        numTokensSellToAddToDOGE = _numTokensSellToAddToDOGE * 10**_decimals;
        emit SwapThresholdsChanged(numTokensSellToAddToLiquidity, numTokensSellToAddToDOGE, _numTokensSellToAddToLiquidity * 10**_decimals, _numTokensSellToAddToDOGE * 10**_decimals);
        return true;
    }
    ...
    function changeTaxForLiquidityAndMarketing(uint256 _taxForLiquidity, uint256 _taxForMarketing) external withTimelock("changeTaxForLiquidityAndMarketing") onlyOwner returns (bool)
    {
        ...
        emit TaxRatesChanged(taxForLiquidity, taxForMarketing, _taxForLiquidity, _taxForMarketing);
        return true;
    }
